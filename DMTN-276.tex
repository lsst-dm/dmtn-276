...
Figure \ref{fig:ex_persistence_Run5} shows an example of this from Run 5 data where three saturated spots persist into subsequent dark images. 
...
and that the persistence was only mitigated via integration time as no persistence was shown in biases. 
...
\begin{equation*}
  Counts= A * \exp(t/\tau)+c
\end{equation*}
where \textit{Counts} are the average number of counts at the location of the spot in the following dark image, \textit{t} is the integration time since the flash, \textit{$\tau$} is the decay constant and \textit{A} and \textit{c} are constants.
...
Though studies are currently being done to minimize or elminate persistence, both through hardware and processing, one of the next steps is to see how much persistence will affect LSST given its current rought characterization.
...
These numbers are similar to what we see with the pinhole data, which we feel will better represent on-sky data.
