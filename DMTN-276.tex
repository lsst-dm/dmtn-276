...
Figure BLANK shows an example of this from Run 6 data where a saturated spot persists into a subsequent dark image. 
...
We tried this and Figure XXX comapares the per amp plot between the nominal voltages and the slightly higher parallel swing voltages.
...
\begin{equation*}
  $Counts= A * \exp(t/\tau)$
\end{equation*}
where \textit{Counts} are the counts in the the dark image, \textit{t} is the integration time since the flash, \textit{\tau} is the decay constant and \textit{A} is a constant.
